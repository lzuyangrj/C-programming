...
from generate_csns_configs import B_SCAN5_GS, POWERS_KW, SIZE_MM  # noqa: E402
...
ION_BEAMS = (
    ("injection", "Injection 80 MeV, H₂⁺"),
    ("extraction", "Extraction 1.6 GeV, H₂⁺"),
)
...
def ion_paths(size_dir: Path, beam: str, sigma_mm: int, power_kw: int):
    on = size_dir / f"csns_{beam}_ions_s{sigma_mm}mm_p{power_kw}kw_sc_on.csv"
...
def main() -> None:
    ...
    for beam, _title in ION_BEAMS:
        for power_kw in POWERS_KW:
            for sigma_mm in SIZE_MM:
                on, off = ion_paths(args.size_dir, beam, sigma_mm, power_kw)
                row = summarize_pair(on, off, f"{beam}_ions", 1000)
                if row is None:
                    continue
                row["power_kw"] = power_kw
    ...
    fig, axes = plt.subplots(1, 2, figsize=(11.4, 4.6), sharey=True)
    for ax, (beam, title) in zip(axes, ION_BEAMS):
        for power_kw in POWERS_KW:
            series = sorted(
                (r for r in srow if r["family"] == f"{beam}_ions" and r["power_kw"] == power_kw),
        ...
        ax.set_title(title)
        ax.set_xlabel("true σ_x [mm]")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
    axes[0].set_ylabel("ion-mode expansion vs no SC [%]")
    fig.tight_layout()
    ...
    for ax, (beam, title) in zip(axes, ION_BEAMS):
        ...
        ax.plot(lims, lims, "k--", lw=0.9, label="σ_IPM = σ_true")
        ax.set_title(title)
        ax.set_xlabel("true σ_x [mm]")
        ax.set_ylabel("detected σ_x with SC [mm]")
        ax.grid(True, alpha=0.3)
        ...
        for sigma_mm in (3, 10, 20):
            ...
            for power_kw in POWERS_KW:
                hit = [
                    r
                    for r in srow
                    if r["family"] == f"{beam}_ions"
                    and r["power_kw"] == power_kw
                    and r["sigma_x_mm"] == sigma_mm
                ]
                if hit:
                    bits.append(f"{power_kw}kW {hit[0]['expansion_vs_no_sc_pct']:+.1f}%")
            if bits:
                print(f"  {title}, {sigma_mm} mm: " + ", ".join(bits))
