...
without changing the hardware. On the Fermilab Booster the inversion is
$\sigma_m=\sigma_0\,h(\sigma_0,N,V)$ with a closed-form $h$ and a
single collected species; the iteration is started at
$\sigma_0=\sigma_m$ and is quoted at $5$--$10\%$ on
$\sigma_0$~\cite{Shiltsev2021}. CSNS cannot copy that recipe. The
machine is in the mixed impulsive/continuous regime
(Section~\ref{sec:mech}), three ToF peaks are resolved, and
$\sigma_m(\sigma_0)$ is not one-to-one. The correction used here is
built from the Virtual-IPM Block~B / I2 tables at $B=0$,
$25\,\mathrm{kV}$.

Figure~\ref{fig:invc}(a) is the collected width versus the true size
for the three species at injection and $100\,\mathrm{kW}$. Each curve has a minimum
($\mathrm{H}_2^+$: $\sigma_m=19.01\,\mathrm{mm}$ at
$\sigma_0=9\,\mathrm{mm}$; $\mathrm{H}_2\mathrm{O}^+$:
$16.01\,\mathrm{mm}$ at $8\,\mathrm{mm}$; $\mathrm{N}_2^+$:
$15.02\,\mathrm{mm}$ at $8\,\mathrm{mm}$). A single measured
$\sigma_m$ above that minimum has two roots. The closed-form
Shiltsev law $\Delta\propto N/(V\sigma_0^2)$ has only the large root
of $\sigma_m=\sigma_0+\kappa/\sigma_0$ and cannot represent the
minimum at all.

A concrete case: injection $\mathrm{H}_2^+$ at $100\,\mathrm{kW}$
gives $\sigma_m=19.16\,\mathrm{mm}$ for a true $10\,\mathrm{mm}$ beam
(dotted line in Fig.~\ref{fig:invc}a). The same collected width is
also produced by $\sigma_0\simeq 8\,\mathrm{mm}$. Between $7$ and
$11\,\mathrm{mm}$ the $\mathrm{H}_2^+$ width changes by only
$0.4\,\mathrm{mm}$, so a single-species inversion is ill-conditioned
exactly at the design core. Starting the Booster iteration at
$\sigma_0=\sigma_m$ selects the large root and returns
$9.7\,\mathrm{mm}$ here only because both roots sit next to the
minimum; for a $5\,\mathrm{mm}$ beam ($\sigma_m=21.3\,\mathrm{mm}$)
the same start returns $14.7\,\mathrm{mm}$.

\begin{figure}[ht]
\centering
\includegraphics[width=0.98\textwidth]{csns_imode_inversion_curves.png}
\caption{Injection, $100\,\mathrm{kW}$, $25\,\mathrm{kV}$, $B=0$.
(a)~Collected $\sigma_m$ versus true $\sigma_0$. The dotted line is
the $\mathrm{H}_2^+$ width of a $10\,\mathrm{mm}$ beam
($19.16\,\mathrm{mm}$); it cuts the $\mathrm{H}_2^+$ curve twice.
(b)~Ratio $\sigma_m(\mathrm{H}_2^+)/\sigma_m(\mathrm{N}_2^+)$,
monotonic in $\sigma_0$.}
\label{fig:invc}
\end{figure}

CSNS resolves the three ToF peaks, so two independent widths are
available on every shot. The ratio
$\sigma_m(\mathrm{H}_2^+)/\sigma_m(\mathrm{N}_2^+)$ falls
monotonically from $1.35$ at $3\,\mathrm{mm}$ to $1.10$ at
$20\,\mathrm{mm}$ (Fig.~\ref{fig:invc}b). That ratio inverts uniquely.
The operational sequence is therefore: identify the $\mathrm{H}_2^+$
and $\mathrm{N}_2^+$ ToF peaks; read $\sigma_m$ of each and the DCCT
bunch charge $N$; interpolate
$R(\sigma_0,N,V)=\sigma_m^{\mathrm{H}_2^+}/\sigma_m^{\mathrm{N}_2^+}$
from the Block~B / I2 table (or from the kick model); return
$\sigma_0=R^{-1}$. Cage voltage and a BPM $\Delta y$ enter as
...
Figure~\ref{fig:invr} applies that correction to the Virtual-IPM
points themselves, leaving the tested $\sigma_0$ out of the
interpolant. Panel~(a), injection $100\,\mathrm{kW}$: the uncorrected
$\sigma_m$ lies far above the diagonal; the single-species large root
recovers $\sigma_0\gtrsim 10\,\mathrm{mm}$ but maps every smaller beam
onto the right-hand branch; the $\mathrm{H}_2^+/\mathrm{N}_2^+$ ratio
recovers $4$--$19\,\mathrm{mm}$ to a median $0.33\%$
(maximum $2.7\%$ at $4\,\mathrm{mm}$). The $10\,\mathrm{mm}$ design
point returns $10.03\,\mathrm{mm}$. Panel~(b): the same two-species
invert at $200$ and $300\,\mathrm{kW}$ stays inside $\pm 2\%$ wherever
the profile is inside the cage ($14$ and $10$ sizes respectively). At
$400$ and $500\,\mathrm{kW}$ every $\mathrm{H}_2^+$ family with
$\sigma_0\le 20\,\mathrm{mm}$ is clipped
($\sigma_m\simeq 42$--$56\,\mathrm{mm}$, detected fraction
$<0.995$) and no table recovers $\sigma_0$.

\begin{figure}[ht]
\centering
\includegraphics[width=0.98\textwidth]{csns_imode_inversion_recover.png}
\caption{(a)~Injection $100\,\mathrm{kW}$: uncorrected $\sigma_m$,
single-species large root, and $\mathrm{H}_2^+/\mathrm{N}_2^+$
inversion, each versus true $\sigma_0$ (leave-one-out).
(b)~Two-species residual at $100$--$300\,\mathrm{kW}$. The grey band
is $\pm 2\%$. Open markers are wall-clipped families.}
\label{fig:invr}
\end{figure}

Aligned extraction at $100\,\mathrm{kW}$ has the same two-valued
$\sigma_m(\sigma_0)$ ($\mathrm{H}_2^+$ minimum $20.4\,\mathrm{mm}$ at
the $10\,\mathrm{mm}$ design size). The I1 size grid is too coarse
($3$, $5$, $7$, $10$, $15$, $20$, $25\,\mathrm{mm}$) for a
leave-one-out ratio invert at the $0.3\%$ injection level; a production
...
$10\%$ level. The $\mathrm{H}_2^+/\mathrm{N}_2^+$ inversion does, at
$100$--$300\,\mathrm{kW}$, provided the ions stay inside the cage.
...
Invert $\sigma_m=\sigma_0\,h(\sigma_0,N,V)$
  ...
  & single species is two-valued;
    $\mathrm{H}_2^+/\mathrm{N}_2^+$ recovers $\sigma_0$ to
    $0.3\%$ (inj.\ $100\,\mathrm{kW}$, $4$--$19\,\mathrm{mm}$);
...
inversion can, but a single ToF peak is two-valued near the design
size; the $\mathrm{H}_2^+/\mathrm{N}_2^+$ ratio recovers $\sigma_0$
to $0.3\%$ at $100\,\mathrm{kW}$ provided the profile has not reached
the wall.
