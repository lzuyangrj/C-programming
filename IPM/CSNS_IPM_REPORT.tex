...
The zeros of $\Delta(B)$ follow Eq.~(\ref{eq:dB}) to $\le 5\%$ at every
cage voltage from $10$ to $30\,\mathrm{kV}$, with no free parameter
(Fig.~\ref{fig:cyc}). At $25\,\mathrm{kV}$ the lobe amplitudes are
$13$, $4$, $1.4$, $0.9$ and $0.6\%$; $300\,\mathrm{G}$ is the first
field at which the envelope itself is $\le 1\%$ for all powers, and is
therefore the operating point rather than any particular zero.

\begin{figure}[ht]
\centering
\includegraphics[width=0.98\textwidth]{csns_review_emode_cyclotron.png}
\caption{Left: $\Delta(B)$ at $10$ and $25\,\mathrm{kV}$
($100\,\mathrm{kW}$, $10\,\mathrm{mm}$ injection) with the predicted
zeros $n\pi m_e/(e\tau)$. Right: spacing between zero crossings versus
cage voltage, compared with $\pi m_e/(e\tau)\propto\sqrt{V}$.}
\label{fig:cyc}
\end{figure}
