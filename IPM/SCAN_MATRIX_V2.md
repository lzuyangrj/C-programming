...
| §2.4 σ = 3 mm extraction is +1.5 % at 300 G; PRAB fit says 320 G; nothing simulated between 300 and 1000 G | **Field fill-in** 400, 500, 600, 800 G for σ = 3–6 mm, both stages, 100–500 kW | E2 |
...
| Axis | v1 | v2 | Reason |
|---|---|---|---|
| e-mode B grid | 0–300 G / 5 G (61), + 1000 G | 14 phase-aware values per voltage (E3); 0/100/200/300/500/1000 G (E1); 400–800 G (E2) | §2.3 zeros predicted; envelope decays by 300 G |
...
| e-mode size | 3–20 mm / 1 mm at 0–300, 1000 G | 3–6 mm at 400–800 G only (rest complete) | fill the 300–1000 G gap for small beams |
...
| **E2** small-beam fill-in | inj. 80 MeV; ext. 1.6 GeV | 3, 4, 5, 6 mm | 400, 500, 600, 800 | 25 / 100–500 | 160 / 32 |
...
| E2 | small beams 3–6 mm at 400–800 G, inj. + ext., 100–500 kW | 192 | 0 | 192 |
...
Reused points are v1 runs that coincide exactly (E1: 80 MeV/120 ns and 1.6 GeV/20 ns at Block A fields; I2: injection sizes 5–25 mm at 100–500 kW). For comparison, v1 executed 8127 runs; v2 adds 15 % of that while closing every "partial" and "open" row of the review matrix that simulation can close.

**Suggested order.** I1 first (it corrects a known bias in published numbers of the report), then I2 (delivers the correction table), E2 (the only e-mode row where 300 G is marginal), E1, E3, E4, I3.
...
4. **New power and field values**: `POWERS_KW` gains 20, 50, 80 (E4, I2) and 150, 250 (I2); `b_tag()` already handles arbitrary gauss values, so 400–800 G (E2) and the phase grids (E3) need only new tuples (`E2_B_GS`, `phase_grid_gs(V)` from `scan_matrix_v2.py`).
